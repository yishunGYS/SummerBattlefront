...
namespace Gameplay.Player.Solider.Attacker.Traverser
{
    public class TraverserFeature : MonoBehaviour
    {
        ...
        [HideInInspector] public bool canTraver; //当前是否可以穿越敌人
        [HideInInspector] public bool isTraver; //当前是否处于穿越状态-穿越状态不可进行攻击/不可被阻挡-到达目标格子/被另一个角色阻挡时退出穿越状态
        [HideInInspector] public GridCell targetCell; //将要穿越到的格子
        [HideInInspector] public float preSpeed; //进入穿越状态前的速度
        [HideInInspector] public EnemyAgent preEnemy; //触发穿越状态的敌方士兵
